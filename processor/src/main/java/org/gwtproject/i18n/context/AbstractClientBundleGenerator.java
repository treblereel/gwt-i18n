...
public abstract class AbstractClientBundleGenerator extends Generator {
    ...
    private void initAndPrepare(TreeLogger logger, GeneratorContext generatorContext, TypeElement bundle) throws UnableToCompleteException {
        ...
        if (bundle == null) {
            ...
            logger.log(TreeLogger.ERROR, MoreTypeUtils.getQualifiedSourceName(bundle)
         ...
         */
        Map<Class<? extends ResourceGenerator>, List<ExecutableElement>> taskList = createTaskList(logger, bundle, generatorContext);
        taskListByTypeElement.put(bundle, taskList);
        /*
         * Additional objects that hold state during the generation process.
         */
        AbstractResourceContext resourceContext = createResourceContext(logger, generatorContext, bundle);
         ...
         * generation.
         */
        Map<ResourceGenerator, List<ExecutableElement>> generators = initAndPrepare(logger, taskList, resourceContext);
    ...
    private Map<Class<? extends ResourceGenerator>, List<ExecutableElement>> createTaskList(
        ...
        ImmutableSet<ExecutableElement> methods = MoreElements.getLocalAndInheritedMethods(sourceType, types, elements);
    ...
    private Map<ResourceGenerator, List<ExecutableElement>> initAndPrepare(
            ...
            AbstractResourceContext resourceContext) throws UnableToCompleteException {
        ...
        for (Map.Entry<Class<? extends ResourceGenerator>, List<ExecutableElement>> entry : taskList.entrySet()) {
    ...
    private boolean initAndPrepare(TreeLogger logger,
        ...
        try {
            ...
            rg.init(logger.branch(TreeLogger.DEBUG, "Initializing ResourceGenerator " + rg.getClass().getCanonicalName()), resourceContext);
        } catch (UnableToCompleteException e) {
            return false;
        }
    ...
    private void process(TreeLogger logger, GeneratorContext generatorContext, TypeElement bundle) throws UnableToCompleteException {
        ...
        Map<ResourceGenerator, List<ExecutableElement>> generators = typeElementResourceGeneratorsMap.get(bundle);
        /*
     ...
     */
    private void finish(TreeLogger logger, AbstractResourceContext
            context, Collection<ResourceGenerator> generators)
            throws UnableToCompleteException {
        boolean fail = false;
     ...
     */
    private boolean createFieldsAndAssignments(TreeLogger logger, AbstractResourceContext resourceContext,
                                               ResourceGenerator rg, List<ExecutableElement> generatorMethods,
                                               SourceWriter sw,
     ...
     */
    private void writeMapMethods(SourceWriter sw, Map<Class<? extends
    ...
    protected static class FieldsImpl implements ClientBundleFields {
        ...
        @Override
        public String define(String rowType, String name, String initializer, boolean isStatic, boolean isFinal) {
        ...
        @Override
        public String define(TypeElement type, String name, String initializer, boolean isStatic, boolean isFinal) {
            return define(type.toString(), name, initializer, isStatic, isFinal);

        }

        @Override
        public String define(ArrayType array, String name, String initializer, boolean isStatic, boolean isFinal) {
