...
public class AnnotationUtil {
   ...
   */
  static <T extends Annotation> T getClassAnnotation(TypeElement clazz,
    ...
    if (annot == null) {
      annot = getClassAnnotation(MoreTypes.asTypeElement(clazz.getSuperclass()), annotationClass);
      if (annot != null) {
        return annot;
      }
      for (TypeMirror intf : clazz.getInterfaces()) {
        annot = getClassAnnotation(MoreTypes.asTypeElement(intf), annotationClass);
        if (annot != null) {
          return annot;
        }
      }
