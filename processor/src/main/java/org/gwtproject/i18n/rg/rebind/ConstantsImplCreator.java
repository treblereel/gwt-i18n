...
class ConstantsImplCreator extends AbstractLocalizableImplCreator {
   ...
   */
  public ConstantsImplCreator(TreeLogger logger, AptContext context, SourceWriter writer,
                              TypeElement localizableClass, ResourceList resourceList)
      throws UnableToCompleteException {
    super(logger, context, writer, localizableClass, resourceList, true);

      register(Type.STRING, new SimpleValueMethodCreator(this,
      ...
      register(Type.INT, new SimpleValueMethodCreator(this,
          SimpleValueMethodCreator.INT));
      register(Type.DOUBLE, new SimpleValueMethodCreator(this,
          SimpleValueMethodCreator.DOUBLE));
      register(Type.FLOAT, new SimpleValueMethodCreator(this,
          SimpleValueMethodCreator.FLOAT));
      register(Type.BOOLEAN, new SimpleValueMethodCreator(this,
          SimpleValueMethodCreator.BOOLEAN));
      register(new Type.ArrayType(String[].class.getCanonicalName(), Type.INT),
  ...
  protected void checkReturnType(TreeLogger logger, ExecutableElement method)
    ...
    if (returnType.getKind().isPrimitive()
        && (returnType.getKind().equals(TypeKind.BOOLEAN)
            ...
            || returnType.getKind().equals(TypeKind.FLOAT) || returnType.getKind().equals(TypeKind.INT))) {
      return;
    }
