...
class ConstantsWithLookupImplCreator extends ConstantsImplCreator {
  ...
  private final Map<ExecutableElement, List<List<ExecutableElement>>> neededPartitionLookups = new HashMap<>();
  ...
  void addNeededPartitionLookups(ExecutableElement targetMethod,
  ...
  void createMethodWithPartitionCheckFor(LookupMethodCreator methodCreator, ExecutableElement targetMethod) {
    List<List<ExecutableElement>> methodPartitions = findMethodsToCreateWithPartitionSize(targetMethod,
  ...
  private void checkMethod(TreeLogger logger, ExecutableElement method)
    ...
    if (getLookupMethodCreator(method.getSimpleName().toString()) != null) {
      ...
      if (params.size() == 0) {
        ...
        if (params.size() != 1
            || !params.get(0).asType().toString().equals(
                "java.lang.String")) {
  ...
  private void createNeededPartitionLookups() {
    for (Entry<ExecutableElement, List<List<ExecutableElement>>> neededPartitionLookup : 
      ...
      LookupMethodCreator lookupMethodCreator = getLookupMethodCreator(targetMethod.getSimpleName().toString());
      List<List<ExecutableElement>> methodForPartitionLookups = neededPartitionLookup.getValue();
      int partitionStartIndex = 0;
      Iterator<List<ExecutableElement>> neededPartitionIterator = methodForPartitionLookups.iterator();
