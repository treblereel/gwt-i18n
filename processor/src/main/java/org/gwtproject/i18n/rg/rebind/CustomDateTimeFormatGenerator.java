...
public class CustomDateTimeFormatGenerator extends Generator {
    ...
    @SuppressWarnings("deprecation")
    private final String generate(TreeLogger logger, final GeneratorContext context,
                                 String typeName, GwtLocale locale) throws UnableToCompleteException {
        TypeElement targetClass = context.getAptContext().elements.getTypeElement(typeName);
        TypeElement oldDateTimeFormat = context.getAptContext().elements.getTypeElement(org.gwtproject.i18n.shared.DateTimeFormat.class.getCanonicalName());
        TypeElement dateTimeFormat = context.getAptContext().elements.getTypeElement(DateTimeFormat.class.getCanonicalName());
        ...
        String className = Util.getJavaClassName(targetClass).replaceAll("\\.", "") + "_"
                + locale.getAsString();
        ...
        if (pw != null) {
            ...
            for (ExecutableElement method : Util.getMethods(targetClass)) {
                ...
                boolean sameNew = context.getAptContext().types.isSameType(returnType, dateTimeFormat.asType());
                boolean sameOld = context.getAptContext().types.isSameType(returnType, oldDateTimeFormat.asType());

                if ( !sameNew && !sameOld) {
                    logger.log(TreeLogger.ERROR, typeName + "." + method.getSimpleName().toString()
                            + " must return DateTimeFormat");
                    throw new UnableToCompleteException();
                }
                String pattern;
                org.gwtproject.i18n.shared.CustomDateTimeFormat.Pattern annotation = method.getAnnotation(org.gwtproject.i18n.shared.CustomDateTimeFormat.Pattern.class);
                if (annotation == null) {
                    org.gwtproject.i18n.client.CustomDateTimeFormat.Pattern oldAnnotation
                            = method.getAnnotation(org.gwtproject.i18n.client.CustomDateTimeFormat.Pattern.class);
                    if (oldAnnotation == null) {
                        logger.log(TreeLogger.ERROR, typeName + "." + method.getSimpleName()
