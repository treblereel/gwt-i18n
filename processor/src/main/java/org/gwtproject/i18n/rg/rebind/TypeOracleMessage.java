...
/**
 * An implementation of the {@link Message} API on
 * top of type oracle.
 */
public class TypeOracleMessage extends AbstractMessage {
    ...
    public TypeOracleMessage(AptContext context, GwtLocaleFactory localeFactory,
                             MessageInterface msgIntf, ExecutableElement method, ResourceList resources) {
    ...
    public <A extends Annotation> A getAnnotation(Class<A> annotClass) {
        ...
        return MoreTypeUtils.getAnnotation(context.types,
    ...
    protected Type mapJTypeToType(TypeMirror type) {
        ...
        if (type.getKind().equals(TypeKind.ARRAY)) {
            ...
            return new Type.ArrayType(componentType.getComponentType() + "[]",
        ...
        if (MoreTypes.asElement(type).getKind().equals(ElementKind.ENUM)) {
            List<Element> list = MoreTypes.asElement(type).getEnclosedElements()
                    .stream()
        ...
        if (date != null && (kind.isClass() || kind.isInterface())
        ...
        if(!type.getKind().equals(TypeKind.TYPEVAR)) {
            ...
            if (list != null && classType != null && context.types.isAssignable(classType.asType(), list.asType())) {
            ...
            if (map != null && classType != null && context.types.isAssignable(classType.asType(), map.asType())
