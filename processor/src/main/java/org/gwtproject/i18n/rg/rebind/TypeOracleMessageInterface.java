...
/**
 * Implementation of the {@link MessageInterface} API
...
public class TypeOracleMessageInterface extends AbstractMessageInterface {
    ...
    public TypeOracleMessageInterface(AptContext context,
                                      GwtLocaleFactory factory,
                                      TypeElement type, ResourceList resources) {
        super(factory);
        this.type = type;
    ...
    public Iterable<Message> getMessages() throws MessageProcessingException {
        ...
        type.getEnclosedElements().stream().filter(elm -> elm.getKind().equals(ElementKind.METHOD))
                .map(elm -> MoreElements.asExecutable(elm)).forEach(method -> {
            messages.add(new TypeOracleMessage(context, factory, this, method,
                                               resources));
