...
public class MoreTypeUtils {
    ...
    public static TypeElement getSuperType(Types types, TypeElement type) {
        List<TypeMirror> list = types.directSupertypes(type.asType())
                .stream()
                .collect(Collectors.toList());
        if (!list.isEmpty() && !list.get(list.size() - 1).toString().equals(Object.class.getCanonicalName())) {
    ...
    public static <A extends Annotation> A getAnnotation(Types types, TypeElement type, Class<? extends Annotation> annotClass) {
    ...
    public static boolean isAnnotationPresent(Types types, TypeElement type, Class<? extends Annotation> annotClass) {
    ...
    public static List<Element> getEnumConstants(TypeElement enumType) {
        return enumType.getEnclosedElements()
                .stream()
